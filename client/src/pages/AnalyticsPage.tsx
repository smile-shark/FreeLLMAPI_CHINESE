...
export default function AnalyticsPage() {
  ...
  const rangeLabel = range === '24h' ? '24 小时' : range === '7d' ? '7 天' : '30 天'
  const spanLabel = spanDays >= 2 ? `${Math.round(spanDays)} 天` : `${Math.max(1, Math.round(spanDays * 24))} 小时`
  const savingsHint =
    `过去 ${rangeLabel} 实际节省 $${actualSavings.toFixed(2)}。这是同等 token 在付费 API 上按模型单价计算的花费。` +
    (extrapolated
      ? `当前数字基于最近 ${spanLabel} 的数据推算至整 30 天。`
      : `当前数字为实际 30 天总额。`)

  const pinned = summary?.pinnedRequests ?? 0
  const pinHonored = summary?.pinHonoredRequests ?? 0
  const requestsHint = pinned > 0
    ? `其中 ${pinned} 次请求指定了特定模型。${pinHonored} 次由指定模型服务；${pinned - pinHonored} 次回退到其他模型。其余为自动路由。`
    : '此期间所有请求均为自动路由；没有客户端指定特定模型。'

  return (
    <div>
      <PageHeader
        title="分析"
        description="请求量、延迟、Token 用量及错误。"
        actions={
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <Stat label="请求数" value={summary?.totalRequests ?? 0} hint={requestsHint} />
          ...
          <Stat label="估算节省" value={`$${savings30d.toFixed(2)}`} hint={savingsHint} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-right">指定</TableHead>
                        <TableHead className="text-right">成功率</TableHead>
                        <TableHead className="text-right">延迟</TableHead>
                        <TableHead className="text-right">输入</TableHead>
                        <TableHead className="text-right">输出</TableHead>
                        <TableHead className="text-right pr-4">节省</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
          ...
          <Panel title="按提供商统计错误">
            {!errorDist?.byPlatform?.length ? (
              <p className="text-sm text-muted-foreground text-center py-8">暂无错误</p>
            ) : (
              <ResponsiveContainer width="100%" height={240}>
