...
export default function EmbeddingsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="模型"
        description="嵌入仅在家族内故障转移：同一模型由另一提供商提供。不同模型的向量不兼容，因此路由器绝不会切换模型。"
        divider={false}
        actions={<ModelsTabs />}
      />
      ...
      <div className="space-y-6">
        <p className="text-xs text-muted-foreground">
          <code className="rounded-md bg-muted px-1.5 py-0.5 font-mono">model: "auto"</code> 在{' '}
          <code className="rounded-md bg-muted px-1.5 py-0.5 font-mono">POST /v1/embeddings</code> 上路由到
          默认家族。指定家族（或提供商模型 ID）将固定该家族；其内部的提供商会按顺序尝试。
        </p>
              ...
              <section key={f.family} className={`rounded-3xl border bg-card p-5 ${noKeys ? 'opacity-60' : ''}`}>
                <div className="flex items-baseline justify-between gap-4 mb-3 flex-wrap">
                  <div className="flex items-baseline gap-2.5 min-w-0">
                    ...
                    {f.maxInputTokens && (
                      <span className="text-[11px] text-muted-foreground/70 tabular-nums">
                        {formatTokens(f.maxInputTokens)} 最大令牌
                      </span>
                    )}
                  ...
                  <span className="text-xs text-muted-foreground tabular-nums">
                    {u ? <>{u.requestsToday} 今日请求 · {formatTokens(u.tokensMonth)} 本月令牌</> : '—'}
                  </span>
                </div>
        ...
        <FloatingBar show={hasChanges}>
          ...
          <Button variant="outline" size="sm" onClick={discard}>放弃</Button>
