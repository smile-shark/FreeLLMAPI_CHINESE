...
const STRATEGIES: { key: RoutingStrategy; label: string; blurb: string }[] = [
  { key: 'priority', label: '手动', blurb: '严格按照下方设置的顺序路由。拖拽手柄调整顺序。不计分，按自上而下顺序依次尝试。' },
  { key: 'balanced', label: '均衡', blurb: '可靠性优先（50%），速度和智能各占 25%。全面的默认方案。' },
  { key: 'smartest', label: '最智能', blurb: '优先选择能力最强且可用的模型。智能 55%，可靠性 35%，速度 10%。' },
  { key: 'fastest', label: '最快', blurb: '优先选择速度最快且可用的模型。速度 55%，可靠性 35%，智能 10%。' },
  { key: 'reliable', label: '最可靠', blurb: '优先保证成功率。可靠性 70%，速度和智能各 15%。' },
  { key: 'custom', label: '自定义', blurb: '通过滑块自定义可靠性、速度和智能的权重。与预设方案使用相同的引擎，权重由你决定。' },
]
...
const WEIGHT_AXES: { key: keyof RoutingWeights; label: string; color: string }[] = [
  { key: 'reliability', label: '可靠性', color: '#22c55e' },
  { key: 'speed', label: '速度', color: '#3b82f6' },
  { key: 'intelligence', label: '智能', color: '#a855f7' },
]
    ...
    <Popover onOpenChange={open => { if (open) { setValues(fromSaved(saved)); setDirty(false) } }}>
      ...
      <PopoverContent align="start" className="w-80">
        <div className="space-y-4">
          <div>
            <h3 className="text-sm font-medium">自定义权重</h3>
    ...
    <section className="rounded-3xl border bg-card p-5">
      ...
      <div className="flex h-2.5 rounded-full overflow-hidden bg-muted">
        {modelsWithWidth.map((m, i) => (
          <div
            key={i}
            title={`${m.displayName} (${m.platform}): 剩余 ${formatTokens(m.remainingTokens)}`}
            style={{
              width: `${m.widthPct}%`,
      ...
      {collapsible && (
        ...
        >
          {expanded ? '收起' : `显示全部 ${models.length} 个模型`}
    ...
    <>
      ...
      <td className="py-2 pr-3 align-middle">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(row.penalty ?? 0) > 0 && (
            <span className="text-[10px] text-amber-600 dark:text-amber-400">−{row.penalty} 惩罚</span>
          )}
          {row.totalRequests !== undefined && row.totalRequests > 0 && (
            <span className="text-[10px] text-muted-foreground/60 tabular-nums">{row.totalRequests} 观测</span>
          )}
        </div>
...
export default function FallbackPage() {
    ...
    <thead>
      <tr className="text-left text-muted-foreground border-b">
        ...
        <th className="py-2 pr-3 font-medium">
          <span className="inline-flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: '#22c55e' }} />可靠性</span>
        </th>
        <th className="py-2 pr-3 font-medium">
          <span className="inline-flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: '#3b82f6' }} />速度</span>
        </th>
        <th className="py-2 pr-3 font-medium">
          <span className="inline-flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: '#a855f7' }} />智能</span>
        </th>
        <th className="py-2 pr-3 font-medium">
          <Tooltip text="始终启用的护栏：免费额度余量 × 实时速率限制惩罚。低于 1.0 表示该模型被限制。">
            <span className="underline decoration-dotted underline-offset-2 cursor-help">护栏</span>
          </Tooltip>
        </th>
        <th className="py-2 pr-3 font-medium text-right">
          <Tooltip text="最终路由分数 = 三个维度的加权平均值，再乘以护栏系数。分数越高越优先路由。">
            <span className="underline decoration-dotted underline-offset-2 cursor-help">分数</span>
          </Tooltip>
        </th>
        <th className="py-2 pr-3 font-medium text-right">开启</th>
      </tr>
    </thead>
  )
  ...
  return (
    <div>
      <PageHeader
        title="模型"
        description="选择路由策略。手动模式下拖拽设置顺序；其他策略根据实时分数（可靠性、速度、智能）进行路由。"
        divider={false}
        actions={<ModelsTabs />}
      />
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-3xl border bg-card p-5">
          <div className="flex items-baseline justify-between mb-3">
            <h2 className="text-sm font-medium">路由策略</h2>
            {routing?.weights && (
              <span className="text-xs text-muted-foreground tabular-nums">
                可靠性 {Math.round(routing.weights.reliability * 100)}% ·
                {' '}速度 {Math.round(routing.weights.speed * 100)}% ·
                {' '}智能 {Math.round(routing.weights.intelligence * 100)}%
              </span>
            )}
          </div>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {isManual
              ? '手动模式：请求按照下方顺序自上而下发送。拖拽可重新排序。'
              : '分数根据实时流量更新。下方为当前请求的路由顺序。'}
          </p>
        </section>
        ...
        {isLoading ? (
          ...
          <div className="rounded-3xl border border-dashed p-8 text-center">
            <p className="text-sm text-muted-foreground">
              没有可用模型。请先在<a href="/keys" className="underline text-foreground">密钥页面</a>添加 API 密钥。
            </p>
          </div>
        ) : (
          <>
            ...
            <FloatingBar show={hasChanges}>
              ...
              <Button variant="outline" size="sm" onClick={() => setLocalEntries(null)}>放弃</Button>
            ...
            {unconfiguredPlatforms.length > 0 && (
              <p className="text-xs text-muted-foreground">隐藏（无密钥）：{unconfiguredPlatforms.join(', ')}</p>
            )}
          </>
        )}
      </div>
