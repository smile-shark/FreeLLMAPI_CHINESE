...
export default function PlaygroundPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessages([...newMessages, {
          role: 'assistant',
          content: `错误：${err.error?.message ?? '未知错误'}`,
        }])
        return
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageHeader
        title="测试场"
        description="通过路由器发送聊天补全请求，查看哪个提供商提供服务。"
        actions={
          <>
            <Select value={selectedModel} onValueChange={(v) => setSelectedModel(v ?? 'auto')}>
              ...
              <SelectContent>
                <SelectItem value="auto">自动（回退链）</SelectItem>
          ...
          {messages.length === 0 ? (
            <div className="flex items-center justify-center h-full text-center">
              <div className="space-y-2 max-w-sm">
                <p className="text-base font-medium">发送消息开始使用。</p>
                <p className="text-sm text-muted-foreground">
                  当前使用 <span className="text-foreground">{activeModelLabel}</span>。在上方选择器中切换模型。
                </p>
              </div>
            </div>
          ) : (
